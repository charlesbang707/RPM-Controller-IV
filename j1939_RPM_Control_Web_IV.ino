...
void sendTSC1() {
    twai_message_t msg;
    msg.identifier = 0x0C000000 |
    ((uint32_t)engineDA << 8) | mySA;
    msg.extd = 1;
    msg.data_length_code = 8;
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pChar) {
        ...
        if (val == 1000) { currentState = RUNNING; targetRPM = 1000; }
        else if (val == 600) { currentState = STOPPING; }
        else if (val == 1) { targetRPM = min(targetRPM + 25, 2500); }
        else if (val == 2) { targetRPM = max(targetRPM - 25, 0); }
        // [수정] Baud Rate 변경 시 저장 후 재부팅
        else if (val == 0x5000) { 
            prefs.putBool("is500k", true); 
            restartDevice(); 
        }
        else if (val == 0x2500) { 
            prefs.putBool("is500k", false); 
            restartDevice(); 
        }
        // Source Address 변경 시에는 재부팅하지 않고 initCAN만 수행 (지시 사항 준수)
...
void loop() {
    ...
    if (now - last100ms >= 100) {
        if (currentState == STOPPING) {
            targetRPM -= 50;
            if (targetRPM <= 0) { currentState = IDLE; targetRPM = 0; }
        }
